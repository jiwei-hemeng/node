...
router.get('/heroeslist', async (req ,res) => {
    ...
    let sql2 = `select * from heroes ${w} order by id limit ${(pagenum-1) * pagesize}, ${pagesize}`;
    ...
    // db('select count(*) cc from heroes' + w, null, (err, result) => {
    //     if (err) return console.log(err);
    //     // console.log(result); // [{cc: 5}]
    //     // 计算总页数
    //     let total = Math.ceil( result[0].cc / pagesize );

    //     // 查询完总记录数，计算完总页数
    //     // 查询数据
    //     let sql = `select * from heroes ${w} order by id limit ${(pagenum-1) * pagesize}, ${pagesize}`;
    //     db(sql, null, (e, r) => {
