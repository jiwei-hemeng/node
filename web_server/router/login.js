...
const secretKey = 'heima-beijing-96';
// 1. 加载express
...
router.post('/reguser', (req, res) => {
    // 1. 接收post请求体（用户提交的数据）
    // console.log(req.body); // {username: 'x', password: 'yy', verify: 'adf'}
    ...
    db(sql, [req.body.username, req.body.password], (err, result) => {
        ...
        if (result.length > 0) {
            ...
            res.json({
                status: 0,
                message: '登录成功',
                token: 'Bearer ' + jwt.sign({ username: req.body.username }, secretKey, { expiresIn: '1h' })
            });
        } else {
